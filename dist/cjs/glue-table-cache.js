...
class GlueTableCache {
    ...
    async ensureS3ListingTable(database, tableName, metadata) {
        if (!this.db)
            await this.connect();
        if (!this.db)
            throw new Error("DB not connected");
        ...
        await this.db.run(`CREATE OR REPLACE TABLE "${tblName}_s3_files" AS ` +
        ...
        await this.db.run(`CREATE OR REPLACE TABLE "${tblName}_s3_listing" AS ` +
            `SELECT path,  ${extractors.join(", ")} FROM "${tblName}_s3_files";`);
        // Create indexes on partition columns
        for (const key of partitionKeys) {
            await this.db.run(`CREATE INDEX IF NOT EXISTS idx_${key} ON "${tblName}_s3_listing" (${key});`);
        }
    }
    async getS3Locations(database, tableName, filter) {
        if (!this.db)
            await this.connect();
        if (!this.db)
            throw new Error("DB not connected");
    ...
    async getFilteredS3Locations(database, tableName, partitionFilters) {
